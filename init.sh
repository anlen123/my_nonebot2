pip install nonebot2
pip install nonebot.adapter.cqhttp
pip install nonebot_plugin_abbrreply
pip install aiohttp
pip install nonebot_plugin_biliav
pip install nonebot_plugin_apscheduler
pip install nonebot_plugin_navicat
pip install selenium
pip install imageio
pip install opencv-python,pendulum,parsel

docker run -d --name redis -p 6379:6379 --restart=always redis --requirepass 123456
pip install redis

mkdir -p ~/QQbotFiles/img
mkdir -p ~/QQbotFiles/pixiv
mkdir -p ~/QQbotFiles/card
mkdir -p ~/QQbotFiles/3c
mkdir -p ~/QQbotFiles/xr
mkdir -p ~/QQbotFiles/ygo
mkdir -p ~/QQbotFiles/yulu
pip install nonebot_plugin_status nonebot_plugin_ygo nonebot_plugin_chess nonebot_plugin_gocqhttp nonebot_plugin_pixiv nonebot_plugin_directlinker nonebot_plugin_repeater
