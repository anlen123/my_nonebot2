...
group.add_argument("-r", "--rule", help="棋局规则，目前支持：五子棋(gomoku)、黑白棋/奥赛罗(othello)")
...
shortcut("停止下棋", ["--stop"], aliases={"认输", "甘拜下风", "投降","结束游戏"}, rule=game_running)
...
async def handle_chess(matcher: Matcher, event: GroupMessageEvent, argv: List[str]):
    ...
    if not games.get(cid, None):
        ...
        if not options.rule:
            await send("@我 + “五子棋”、“黑白棋”、开始一局游戏。")

        if options.rule not in rules:
            await send("没有找到对应的规则，目前支持：围棋(go)、、黑白棋(othello)")
