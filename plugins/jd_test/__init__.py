...
async def jdd_add_wwj():
    ...
    headers = {
      'user-agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.198 Safari/537.36',
      'referer': 'https://wqs.jd.com/my/jingdou/my.shtml?sceneval=2&jxsid=16399181398606646767&ptag=7155.1.17',
      'cookie': 'pt_key=AAJiCi4lADBcaDlE45ePPg_85HjTyOKuPXadOAWiY-1LcrrCmAZY9RC6YqOkvpcA1kX816jRWZg;pt_pin=1255542159_m;'
    }
    now = datetime.now().hour
    if now >=1 and now <=10 :
        pass
    ...
    if jdd and jdd_count_wwj and int(jdd_count_wwj)-int(jdd)!=0:
        clien.set("jdd_count_wwj",jdd)
        bot = nonebot.get_bots()
        if bot :
            bot = bot['1928994748']
            await bot.send_msg(message=f"京东豆增加{int(jdd)-int(jdd_count_wwj)}个,现在你的京东豆数量是: {jdd}个",user_id="1255542159")
