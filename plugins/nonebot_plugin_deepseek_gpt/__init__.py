...
from nonebot import get_driver, on_command, on_keyword, on_message, on_startswith
...
async def deepseek(message: str, user_id: str, model: str) -> list:
    client = AsyncOpenAI(
        api_key="77b1cd88-22d3-4c9e-9091-8394d5bbfcab",
...
async def deepseek_singe_rev(event: Event, bot: Bot):
    ...
    try:
        mess = await bot.send(
            event=event, message=MessageSegment.text(f"{model}正在思考......")
        )
...
async def clear_conversation(event: Event, bot: Bot):
    ...
    # 返回清除成功的提示
    await bot.send(
        event=event, message=MessageSegment.text("您的deepseek对话记录已被清除！")
    )
...
async def ds_fy_singe_rev(event: Event, bot: Bot):
    if event.get_plaintext().startswith("fy ") or event.get_plaintext().startswith(
        "翻译 "
    ):
...
async def deepseek_fy(content: str) -> list:
    ...
    messages = [
        {
            "role": "system",
            "content": "你现在扮演翻译官，你可以准确的翻译日文，英文，韩文，英语为中文。当我输入中文的时候则翻译成英文，只需要给我结果即可，无需说多余的话。",
        },
